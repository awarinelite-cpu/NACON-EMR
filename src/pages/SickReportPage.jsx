...
export default function SickReportPage() {
  ...
  // ── All unique classes from today's sick reports ──────
  const allClasses = [...new Set(sickReports.map(p => p.classSet).filter(Boolean))].sort();

  // ── Apply class filter first, then tab ───────────────
  const byClass = (list) =>
  ...
  // ── Per-class counts (always from full sickReports) ──
  const classCounts = {};
  sickReports.forEach(p => {
    const c = p.classSet || '—';
  ...
  const pillStyle = (val) => ({
    ...
    background: classFilter === val ? '#f97316' : 'var(--card-bg2)',
  ...
  return (
    <div style={{ display:'flex', flexDirection:'column', minHeight:'100%' }}>
      ...
      <div className="page-content" style={{flex:1}}>
        ...
        {/* ── Per-class breakdown (collapsed when filtered) ── */}
        {classFilter === 'all' && allClasses.length > 1 && (
          <div style={{
            ...
            <div style={{fontSize:10, fontWeight:700, color:'var(--t3)', marginBottom:8, textTransform:'uppercase', letterSpacing:'.06em'}}>
              <i className="ti ti-chart-bar" style={{marginRight:4}} />Sick Reports by Class
            </div>
